...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_color__ = __webpack_require__(0);
...
function startGame() {
	if (game === true) return;else game = true;
	...
	function onEnd() {
		...
		gameInstance = new __WEBPACK_IMPORTED_MODULE_0__game__["a" /* default */]({ ctx, canvas, onEnd });
		updatePlayers();
	}

	let gameInstance = new __WEBPACK_IMPORTED_MODULE_0__game__["a" /* default */]({ ctx, canvas, onEnd });
	...
	function newPlayer(e) {
		const keys = [{ jump: "Space", right: "KeyD", left: "KeyA" }, { jump: "Numpad0", right: "ArrowRight", left: "ArrowLeft" }, { jump: "KeyI", right: "KeyL", left: "KeyJ" }][gameInstance.Players.length];
		const color = [__WEBPACK_IMPORTED_MODULE_1_color___default()("#9a4c95"), __WEBPACK_IMPORTED_MODULE_1_color___default()("#1d1a31"), __WEBPACK_IMPORTED_MODULE_1_color___default()("#d2d4c8"), __WEBPACK_IMPORTED_MODULE_1_color___default()("#a5243d"), __WEBPACK_IMPORTED_MODULE_1_color___default()("#b9cfd4")][gameInstance.Players.length];
...
"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_color__ = __webpack_require__(0);
...
class Block {
	constructor({ pos }) {
		...
		this.color = __WEBPACK_IMPORTED_MODULE_0_color___default()("#B8E4F9").desaturate(0.5).hex();
	...
	isColliding(obj) {
		return this.pos.x + this.width > obj.pos.x && this.pos.x < obj.pos.x + obj.width && this.pos.y < obj.pos.y + obj.height && this.pos.y > obj.pos.y;
	}
	draw({ ctx, Viewport }) {
...
"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__block_js__ = __webpack_require__(3);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__player_js__ = __webpack_require__(5);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_color__ = __webpack_require__(0);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_color___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_2_color__);



window.Color = __WEBPACK_IMPORTED_MODULE_2_color___default.a;
...
class Game {
	constructor({ ctx, canvas, onEnd }) {
		...
		this.Viewport = {
			...
			Update(pos) {
				...
				if (this.x < 0) this.x = 0;else if (this.x + this.width > game.Map.width) this.x = game.Map.width - this.width;
		...
		this.Water = {
			progress: -200,
			progression: 0.5,
			color: __WEBPACK_IMPORTED_MODULE_2_color___default()("#9FDDF9").alpha(0.4),
			draw(ctx) {
				ctx.fillStyle = this.color;
	...
	get maxScore() {
		return this.Players.sort((e, a) => e.score < a.score)[0].score;
	}
	...
	AddPlayer({ name, keys, color }) {
		const player = new __WEBPACK_IMPORTED_MODULE_1__player_js__["a" /* default */]({ color, keys, name, pos: { x: this.Map.width / 2, y: 0 } });
		player.alive = true;
		this.Players.push(player);
	}
	AddBlock(pos) {
		this.Blocks.push(new __WEBPACK_IMPORTED_MODULE_0__block_js__["a" /* default */](pos));
	}
	ToggleLive() {
		this.live = !this.live;
	...
	Update() {
		...
		this.Blocks.map(block => {
			...
			this.Blocks.filter(b => b !== block).filter(block.isColliding.bind(block)).map(colblock => {
		...
		this.Players.filter(e => e.alive).forEach(player => {
			...
			// Player x
			if (player.vel.x > player.maxSpeed) player.vel.x = player.maxSpeed;else if (player.vel.x < -player.maxSpeed) player.vel.x = -player.maxSpeed;
...
class Player {
	constructor({ pos = { x: 50, y: 0 }, name = "Player", keys = { jump: "Space", left: "KeyA", right: "KeyD" }, color = __WEBPACK_IMPORTED_MODULE_0_color___default()("red") }) {
		this.name = name;
		this.pos = pos;
		this.vel = {
			x: 0,
	...
	IsColliding(obj) {
		return this.pos.x + this.width > obj.pos.x && this.pos.x < obj.pos.x + obj.width && this.pos.y < obj.pos.y + obj.height && this.pos.y + this.height > obj.pos.y;
	}
	draw({ ctx, Viewport }) {
		if (!this.alive) return;

		ctx.fillStyle = this.onWall ? __WEBPACK_IMPORTED_MODULE_0_color___default()(this.color).darken(0.3) : this.color;
